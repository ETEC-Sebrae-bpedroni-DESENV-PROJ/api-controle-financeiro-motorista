...
import br.gov.sp.etecsebrae.service.MarcaVeiculoService;

@RestController
@RequestMapping({ "/marca-veiculo", "/marca_veiculo" })
public class MarcaVeiculoController {
	...
	public ResponseEntity<?> getAll() {
		try {
			List<MarcaVeiculo> list = service.getAll();
	...
	public ResponseEntity<?> save(@RequestBody MarcaVeiculo dto) {
		try {
			dto.setId(0);
	...
	public ResponseEntity<?> update(@RequestBody MarcaVeiculo dto) {
		try {
	...
	public ResponseEntity<?> updateById(@PathVariable int id) {
		try {
			MarcaVeiculo dto = service.getById(id);
			return ResponseEntity.ok(service.save(dto));
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(e.getMessage());
		}
	...
	public ResponseEntity<?> delete(@RequestBody MarcaVeiculo dto) {
		try {
			service.delete(dto);
