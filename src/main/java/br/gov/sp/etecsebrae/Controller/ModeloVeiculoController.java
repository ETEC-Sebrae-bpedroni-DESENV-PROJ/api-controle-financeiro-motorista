...
public class ModeloVeiculoController {
	...
	public ResponseEntity<?> save(@RequestBody ModeloVeiculo dto) {
		try {
			dto.setId(0);
	...
	public ResponseEntity<?> update(@RequestBody ModeloVeiculo dto) {
		try {
	...
	public ResponseEntity<?> updateById(@PathVariable int id) {
		try {
			ModeloVeiculo dto = service.getById(id);
			return ResponseEntity.ok(service.save(dto));
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(e.getMessage());
		}
	...
	public ResponseEntity<?> delete(@RequestBody ModeloVeiculo dto) {
		try {
			service.delete(dto);
