...
import br.gov.sp.etecsebrae.service.TipoVeiculoService;

@RestController
@RequestMapping({ "/tipo-veiculo", "/tipo_veiculo" })
public class TipoVeiculoController {
	...
	public ResponseEntity<?> getAll() {
		try {
			List<TipoVeiculo> list = service.getAll();
	...
	public ResponseEntity<?> save(@RequestBody TipoVeiculo dto) {
		try {
			dto.setId(0);
	...
	public ResponseEntity<?> update(@RequestBody TipoVeiculo dto) {
		try {
	...
	public ResponseEntity<?> updateById(@PathVariable int id) {
		try {
			TipoVeiculo dto = service.getById(id);
			return ResponseEntity.ok(service.save(dto));
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(e.getMessage());
		}
	...
	public ResponseEntity<?> delete(@RequestBody TipoVeiculo dto) {
		try {
			service.delete(dto);
