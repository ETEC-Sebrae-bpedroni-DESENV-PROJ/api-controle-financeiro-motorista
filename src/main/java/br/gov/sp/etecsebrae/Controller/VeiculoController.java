...
import br.gov.sp.etecsebrae.service.VeiculoService;
...
public class VeiculoController {
	...
	public ResponseEntity<?> save(@RequestBody Veiculo dto) {
		try {
			dto.setId(0);
	...
	public ResponseEntity<?> update(@RequestBody Veiculo dto) {
		try {
	...
	public ResponseEntity<?> updateById(@PathVariable int id) {
		try {
			Veiculo dto = service.getById(id);
			return ResponseEntity.ok(service.save(dto));
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(e.getMessage());
		}
	...
	public ResponseEntity<?> delete(@RequestBody Veiculo dto) {
		try {
			service.delete(dto);
